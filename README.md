...
In this repository we will be deploying a voting app, using AWS EKS cluster, MongoDB, Golang, UI(It's designed to be accessible to users via the internet, allowing them to vote for their preferred programming language out of six choices: C#, Python, JavaScript, Go, Java, and NodeJS.) by React and JS
...
```
git clone https://github.com/YASWANTH-043/Deploying-a-Voting-App-on-AWS-EKS.git
```

**Create CloudCluster Namespace**
```
kubectl create ns cloudcluster

kubectl config set-context --current --namespace cloudcluster
```

**MONGO Database Setup**


To create Mongo statefulset with Persistent volumes, run the commands in Mongo-statefulset.yaml folder:
```
